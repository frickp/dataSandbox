...
nm = names(Boston)
nm = nm[nm!='medv']
# And then, I need to selectively remove the variable names from nm as the selection iterates




rev.model=steo(object=)


# Create a list that contains all of the predictors chosen
M <- ('lstat')
M <- append(M,'secondPred')
...
# So, call the name of the first model '1'
...
Do forward, reverse, and mixed selection on a linear model.
...
fwd.model=step(object=min.model,scope=formula(max.model),direction='both')
...
This is a pre-loaded example showing how you can suppress the code used to generate the plots
```{r, echo=FALSE}
plot(cars)
```
